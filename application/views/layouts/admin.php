  ...
  <section class="px-5 py-3 w-full md:max-w-[280px] fixed bg-gray-800 md:min-h-screen md:shadow-lg md:shadow-gray-700 text-sm font-semibold">
    <nav class="flex py-3 justify-between items-center"><a href="#" class="w-full md:justify-normal"><div class="flex py-2 justify-center rounded-md w-full items-center text-blue hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-lg hover:shadow-gray-900 transition"><img src="/public/images/logo.svg" class="w-12" alt="Deepmind logo"><span class="pl-3 font-semibold text-3xl">DME l Panel</span></div></a>
      ...
      <div class="sidebar hidden md:block space-y-4 mt-6">
        ...
        <nav class="flex justify-between">
          <a class="nav-link flex items-center <?php if ($this->route['action'] == 'statistic')echo 'active' ?>" href="#">
            <svg xmlns="http://www.w3.org/2000/svg" class="mr-2" width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
              <path
  ...
  <section role="main" class="w-full md:ml-80 pt-12 md:pt-6 min-h-screen px-3 sm:px-10">
    ...
    <div class="border-l-4 mt-3 border-blue pt-2 pb-3 pl-3 md:w-full lg:w-2/3 xl:w-1/2 rounded-r-sm shadow-md">
      ...
      <ul class="text-sm list-disc list-inside ml-3">
        ...
        <li>Системные настройки контроллера admin доступны только для чтения.<br>Они не будут перезаписаны. Их возможно отредактировать только вручную</li>
        <li>Имеются расширенные настройки для опытных пользователей внизу страницы.<br>Изменяйте их на ствой страх и риск!</li>
      </ul>
    ...
    <div class="border-l-4 mt-3 border-yellow pt-2 pb-3 pl-3 sm:w-4/5 md:w-2/3 xl:w-1/2 rounded-r-sm shadow-md">
      <p class="mt-1 text-sm self-center leading-6 pr-1">Внимание, при изменении параметров через интерфейс панели возможна перезапись индивидуальных донастроек! Рекомндуется сделать резервную копию данных</p>
    </div>
    <div id="info">
        <div class="border-l-4 mt-3 border-red pt-2 pb-3 pl-3 sm:w-4/5 md:w-2/3 xl:w-1/2 rounded-r-sm shadow-md">
          <p class="mt-1 text-sm self-center leading-6 pr-1">Ошибка: произошла ошибка при обновлении конфигурации. Проверьте права доступа<br>на чтение/запись файлов конфигурации ACL 'application/acl/{controller}.acl.php'</p>
        </div>
    </div>
    <!-- end info -->
    ...
    <div class="w-full lg:w-2/3 xl:w-1/2">
      ...
      <div class="col-span-6 md:col-span-4">
        ...
        <select class="bg-input min-w-min border-blue focus:border-blue rounded-md" onchange="renderActions(this);" id="controllers_select">
        ...
        <div name="acl" class="space-y-4 px-3 py-2 rounded-sm">
          <div class="">
            ...
            <div class="border-x pt-5 border-blue rounded-md py-3 px-6 space-y-2 select-none shadow-md" id="acl_all"
              ...
              <!-- users -->
              <div class="border hidden only:block rounded-md p-1 border-[inherit] px-6 text-desc bg-gray-800">Поместите ваш action здесь...</div>
            </div>
          </div>
          ...
          <div class="">
            <label for="acl_authorize" class="absolute ml-7 text-sm font-medium leading-6 text-text">Авторизированные
              пользователи:</label>
            <div class="border-x pt-7 border-green rounded-md py-3 px-6 space-y-2 select-none shadow-md" id="acl_authorize"
          ...
          <div class="">
            <label for="acl_admin" class="absolute ml-7 text-sm font-medium leading-6 text-text">Администраторы сисемы:</label>
            <div class="border-x pt-7 border-red rounded-md py-3 px-6 space-y-2 select-none shadow-md" id="acl_admin"
              ondragover="onDragOver(event);" ondrop="onDrop(event);" ondragenter="onDragEnter(event);">
              <!-- admin -->
              <div class="border hidden only:block rounded-md p-1 border-[inherit] px-6 text-desc bg-gray-800">Поместите ваш action здесь...
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="list col-span-6 md:col-span-4">
        <details class="open:bg-input open:shadow-lg p-6 rounded-lg">
          <summary class="text-sm leading-6 text-gray-600 dark:text-white font-semibold select-none">
            Расширеный режим<br>(Изменяйте содержимое только тогда, когда <span class="text-blue underline">полностью уверены
